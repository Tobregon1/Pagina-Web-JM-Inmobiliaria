document.addEventListener('DOMContentLoaded', () => {
    ...
    function toggleFavorite(id, event) {
        ...
        // Si estamos en la vista de favoritos, removemos la tarjeta del DOM directamente
        if (currentFilter.operation === 'favorites' && index !== -1) {
            const card = btn.closest('.property-card');
            if (card) {
                card.remove();
                if (propertiesGrid.querySelectorAll('.property-card').length === 0) {
                    propertiesGrid.innerHTML = '<div class="no-results" style="grid-column: 1/-1; text-align: center; padding: 40px; color: #666;">No tenés propiedades favoritas guardadas.</div>';
                }
            }
        }
    }
    ...
    function renderProperties(filter, keepPage = false) {
        if (!keepPage) {
            showLoading();
            currentPage = 1;
        }
        ...
        setTimeout(() => {
            ...
            const propsToRender = filteredProps.slice(startIndex, endIndex);

            propsToRender.forEach((prop, index) => {
                ...
                if (imageSrc) {
                    imageHtml = `<div class="property-image-container" style="height: 250px; overflow: hidden;">
                                    <img src="${imageSrc}" alt="${prop.title}" loading="lazy" width="400" height="250" style="width: 100%; height: 100%; object-fit: cover; transition: transform 0.5s ease;">
                                 </div>`;
                } else {
            ...
            if (!paginationContainer) {
                ...
                paginationContainer.style.paddingTop = '20px';
                paginationContainer.style.borderTop = '1px solid var(--border-color)';
                paginationContainer.style.gridColumn = '1 / -1';
                propertiesGrid.parentNode.appendChild(paginationContainer);
            }

            if (totalItems > propertiesPerPage) {
                paginationContainer.style.display = 'flex';
                paginationContainer.innerHTML = `
                    <button class="search-btn" id="prevPageBtn" style="padding: 10px 20px; font-size: 15px; ${currentPage === 1 ? 'opacity: 0.5; pointer-events: none;' : ''}">← Anterior</button>
                    <span style="color: var(--text-color); font-weight: 500; font-size: 15px;">Mostrando ${startIndex + 1}-${Math.min(endIndex, totalItems)} de ${totalItems} propiedades</span>
                    <button class="search-btn" id="nextPageBtn" style="padding: 10px 20px; font-size: 15px; ${currentPage === totalPages ? 'opacity: 0.5; pointer-events: none;' : ''}">Siguiente →</button>
                `;
                ...
                if (prevBtn) {
                    prevBtn.addEventListener('click', () => {
                        if (currentPage > 1) {
                            currentPage--;
                            renderProperties(currentFilter, true);
        ...
        }, keepPage ? 0 : 500); // Simulate network delay solo si no es un refresco de página
    }
    ...
    function openModal(prop) {
        ...
        if (images.length > 1) {
            ...
            images.forEach((img, index) => {
                slidesHtml += `
                    <div class="carousel-slide ${index === 0 ? 'active' : ''}">
                        <img src="${img}" alt="${prop.title} - Foto ${index + 1}" loading="lazy" width="800" height="600" style="width: 100%; height: 100%; object-fit: cover;">
                    </div>
                `;
            ...
            // Single Image
            placeholder.innerHTML = `<img src="${images[0]}" alt="${prop.title}" loading="lazy" width="800" height="600" style="width: 100%; height: 100%; object-fit: cover;">`;
        } else {
            // No Image
